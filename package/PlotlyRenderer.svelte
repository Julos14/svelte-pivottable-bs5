<script>
    ...
    $: {
        ...
        if (numInputs !== 0) {
            fullAggName += ` ${pivotData.getLocaleStrings()['of']} ${pivotData.props.vals.slice(0, numInputs).join(', ')}`;
        }
        ...
        if (hAxisTitle !== '') {
            titleText += ` ${pivotData.getLocaleStrings()['vs']} ${hAxisTitle}`;
        }
        if (groupByTitle !== '') {
            titleText += ` ${pivotData.getLocaleStrings()['by']} ${groupByTitle}`;
        }
